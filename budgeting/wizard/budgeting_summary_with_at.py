...
class BudgetingSummaryAT(models.TransientModel):
    _name = "budgeting.summary.at"
    _description = "Budgeting Summary With Accounts"
    ...
    partner_ids = fields.Many2many('res.partner', string='Partners', default=lambda self: self.env['res.partner'].search([]))
    date_to = fields.Date(string='As Of')
    dyear = fields.Char(string='Year')
    rep_ids = fields.One2many('budgeting.app.stmt.wizard.summary.at', 'rep_id')
    period_fill = fields.Boolean('Fill Budget Specs', default=True)
    source_fill = fields.Boolean('Fill Source', default=True)
    partner_fill = fields.Boolean('Fill Partner', default=True)
    ...
    @api.onchange('partner_fill')
    def onchange_partner_fill(self):
        for rec in self:
            if rec.partner_fill:
                rec.partner_ids = self.env['res.partner'].search([])
            else:
                rec.partner_ids = self.env['res.partner'].search([('id', '=', 0)])
    ...
    def _print_report(self, data):
        ...
        # return self.env['report'].get_action(self, 'budgeting.appstmt_report_summary_at', data=data)
        return self.env.ref('budgeting.appstmt_summary_at').report_action(self, data=data)
    ...
    def check_report(self):
        ...
        data['form'] = self.read(['source_ids', 'period_ids', 'date_to','dyear','partner_ids'])[0]
    ...
    def group_source(self):
        ...
        sql = """ 
            ...
            FROM budgeting_app_stmt_wizard_summary_at det
            where rep_id = %s  
...
class AppStmtWizardSummaryAT(models.TransientModel):
    _name = "budgeting.app.stmt.wizard.summary.at"
    _description = "Appropriation Summary With Accounts"

    rep_id = fields.Many2one('budgeting.summary.at', string="Report", copy=False)
...
class AppStmtReportSummaryAT(models.AbstractModel):
    """Abstract Model for report template.
    for `_name` model, please use `report.` as prefix then add `module_name.report_name`.
    """

    _name = "report.budgeting.appstmt_report_summary_at"

    @api.model
    def render_html(self, docids, data=None):
        self.model = self.env.context.get('active_model')
        self.get_report_values(self.env.context.get('active_id'),data)
        docs = self.env['budgeting.summary.at'].browse(self.env.context.get('active_id'))
        docargs = {
            'doc_ids': self.ids,
        ...
        # return self.env['report'].render('budgeting.appstmt_report_summary_at', docargs)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        partner_id = data['form']['partner_ids']
        ...
        if partner_id:
            if len(partner_id) == 1:
                cond += ' and partner_id = %s' % str(partner_id[0])
            else:
                cond += ' and partner_id in %s' % str(tuple(partner_id)) 
        ...
        if partner_id:
            if len(partner_id) == 1:
                cond2 += ' and budgeting_obr.partner_id = %s' % str(partner_id[0])
            else:
                cond2 += ' and budgeting_obr.partner_id in %s' % str(tuple(partner_id))
                
        cond2 += " and budgeting_obr.date_approved <= '" + dateto + "'::date "
                
        docs = []
        query = ("""
                select source_id, period_id, partner_id, sum(allotment) as allotment, sum(incurred) as incurred,
                ...
                from 
                    (SELECT 'APP' source, budgeting_appropriationdet.amount, 
                                ...
                                budgeting_appropriation.source_id, budgeting_appropriation.partner_id,
                ...
                """ + cond + """ 
                    ...
                    SELECT 'OBR' source, budgeting_obrdet.amount, 
                                ...
                                budgeting_obr.source_id, budgeting_obr.partner_id,
                    ...
                    SELECT 'AUF' source, budgeting_augmentationdetfrom.amount amount, 
                                ...
                                budgeting_augmentation.source_id, budgeting_augmentation.partner_id,
                    ...
                    """ + cond +
                    """ 
                    order by source_id, partner_id, period_id) as src
                    group by source_id, partner_id, period_id 
                        ;
                """)

        self.env.cr.execute(query, (cond,cond,cond,cond,cond))
        query_results = self.env.cr.dictfetchall()
        self.env["budgeting.app.stmt.wizard.summary.at"].search([('rep_id','=',self.env.context.get('active_id'))]).unlink()
        
        rep_obj = self.env['budgeting.app.stmt.wizard.summary.at']
